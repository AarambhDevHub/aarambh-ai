...
impl SafetyGenerator for SpeculativeEngine {
    fn generate(&mut self, prompt: &str, config: GenerationConfig) -> Result<GenerationOutput> {
        SpeculativeEngine::generate(self, prompt, config)
    }
...
mod tests {
    use super::*;
    use aarambh_studio_inference::{FinishReason, GenerationPhase, Sampler, ToolCall};
    ...
    fn output(text: &str) -> GenerationOutput {
        GenerationOutput {
            ...
            tool_call: None,
            usage: aarambh_studio_inference::GenerationUsage::default(),
        }
    }

...
impl GroupedQueryAttention {
    ...
    pub(crate) fn attend_projected(
        ...
        let out = if training && mask.is_some() {
            aarambh_studio_kernel::dispatch::attention_forward_candle(&q, &k, &v, mask, self.scale)?
        } else if mask.is_some() {
            // DSA supplies a general additive mask that must not be collapsed
            // into the boolean causal fast-path.
            aarambh_studio_kernel::dispatch::attention_forward_additive(
                &q,
                &k,
                &v,
    ...
    pub(crate) fn attend_projected_sparse(
        ...
        let v = v.transpose(1, 2)?.contiguous()?;
        let out = if training {
            aarambh_studio_kernel::attention_forward_candle(&q, &k, &v, Some(mask), self.scale)?
        } else {
            aarambh_studio_kernel::dsa_sparse_attention_forward(
                &q,
                &k,
                &v,
                mask,
    ...
    pub fn forward_decode_batch(
        ...
        for row in 0..batch {
            ...
            let out = aarambh_studio_kernel::dispatch::attention_forward_causal(
    ...
    pub fn forward_train(
        ...
        let out = match mask {
            Some(mask) => aarambh_studio_kernel::dispatch::attention_forward_train(
                &q,
                &k,
                &v,
                Some(mask),

...
fn apply_env_overrides(config: &mut DistributedConfig) -> Result<()> {
    if let Some(world_size) = env_usize("AARAMBH_WORLD_SIZE")? {
        config.world_size = world_size;
        config.enabled = world_size > 1;
    }
    if let Some(rank) = env_usize("AARAMBH_RANK")? {
        config.rank = rank;
    }
    if let Some(local_rank) = env_usize("AARAMBH_LOCAL_RANK")? {
        config.local_rank = local_rank;
    }
    if let Ok(run_id) = env::var("AARAMBH_DIST_RUN_ID")
        && !run_id.trim().is_empty()
    {
        config.run_id = run_id;
    }
    if let Ok(path) = env::var("AARAMBH_DIST_RENDEZVOUS")
        && !path.trim().is_empty()
    {

...
use aarambh_ai_safety::{SafetyInspector, SafetyPolicy, SafetyVerdict};
use aarambh_ai_tokenizer::{ASSISTANT, USER};
...
impl ChainDecoder for EvalChainDecoder {
    fn context_limit(&self) -> usize {
        aarambh_ai_core::Configurable::config(self.engine.model()).max_seq_len
    }
    ...
    fn generate(
        &mut self,
        transcript_ids: &[u32],
        _pending_media: Option<&aarambh_ai_agent::ToolResultContent>,

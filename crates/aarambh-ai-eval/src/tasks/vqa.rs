...
use aarambh_ai_tokenizer::{IMAGE, IMAGE_END, IMAGE_ID};
...
/// Vision-question-answering task for Phase 20 VLM checkpoints.
pub struct VqaTask;

impl EvalTask for VqaTask {
    ...
    fn run(&self, context: &EvalContext, config: &EvalConfig) -> Result<TaskScore> {
        let data_path = first_existing(&[
            config.data_dir.join("vqa").join("data.jsonl"),
            config.data_dir.join("vqa_smoke").join("data.jsonl"),
            config.data_dir.join("vqa.jsonl"),
        ])?;
        let examples = read_jsonl::<VqaEvalExample>(&data_path, config.max_examples)?;
        ...
        for example in &examples {
            ...
            let prompt =
                ensure_image_prompt(&format!("<|user|>\n{}\n<|assistant|>\n", example.question));
            ...
            if matches_vqa_expected(&output, &example.answers, &example.keywords) {
                passed += 1;
            }
        }

        Ok(TaskScore::accuracy("vqa", passed, examples.len()))
    }
}
...
pub(crate) fn load_vision_runtime(
    ...
    let config_path = config
        .config_path
        .as_ref()
        .ok_or_else(|| AarambhError::Config("VQA eval requires --config".into()))?;
    let run_config = TrainingRunConfig::from_toml(config_path)?;
    let vision = run_config
        .vision
        .as_ref()
        .ok_or_else(|| AarambhError::Config("VQA eval requires [vision] config".into()))?;
...
pub(crate) fn greedy_generate_from_embeddings(
    ...
    if prompt_len >= context.max_seq_len() {
        return Err(AarambhError::Shape(format!(
            "VQA prompt length {prompt_len} leaves no room in max_seq_len {}",
            context.max_seq_len()
        )));
    }
...
fn resolve_image_path(root: &Path, example: &VqaEvalExample) -> Result<PathBuf> {
    let path = example
        .image_path
        .as_ref()
        .or(example.image.as_ref())
        .ok_or_else(|| AarambhError::Config("VQA example is missing image".into()))?;
    if path.is_absolute() {
        Ok(path.clone())
    } else {
        Ok(root.join(path))
...
fn matches_vqa_expected(output: &str, answers: &[String], keywords: &[String]) -> bool {
    let output = normalize_answer(output);
    if output.is_empty() {
        return false;
    }
    if answers
        .iter()
        .any(|answer| output == normalize_answer(answer))
    {
        return true;
    }
    if keywords.is_empty() && answers.is_empty() {
        return true;
    }
    keywords
        .iter()
        .any(|keyword| output.contains(&normalize_answer(keyword)))
}
...
mod tests {
    ...
    #[test]
    fn vqa_exact_answer_ignores_case_and_punctuation() {
        assert!(matches_vqa_expected("Red.", &[String::from("red")], &[]));
    }

    #[test]
    fn vqa_keyword_match_accepts_partial_answer() {
        assert!(matches_vqa_expected(
            "It is a red square.",
            &[],
            &[String::from("red")]
        ));

...
fn main() {
    ...
    let kernels = [
        (
            "AARAMBH_STUDIO_CUDA_FLASH_ATTN_PTX",
            "flash_attention",
            "kernels/flash_attention.cu",
        ),
        (
            "AARAMBH_STUDIO_CUDA_FLASH_ATTN_BWD_PTX",
            "flash_attn_bwd",
            "kernels/flash_attn_bwd.cu",
        ),
        (
            "AARAMBH_STUDIO_CUDA_RMS_NORM_PTX",
            "rms_norm_fused",
            "kernels/rms_norm_fused.cu",
        ),
        (
            "AARAMBH_STUDIO_CUDA_ROPE_PTX",
            "rope_apply",
            "kernels/rope_apply.cu",
        ),
        (
            "AARAMBH_STUDIO_CUDA_SWIGLU_PTX",
            "swiglu_fused",
            "kernels/swiglu_fused.cu",
        ),
        (
            "AARAMBH_STUDIO_CUDA_GATED_DELTA_PTX",
            "gated_delta_recurrent",
            "kernels/gated_delta_recurrent.cu",
        ),
        (
            "AARAMBH_STUDIO_CUDA_DSA_PTX",

use aarambh_studio_kernel::cpu::parallel_attn::{cpu_parallel_attn, cpu_sequential_attn};
use aarambh_studio_kernel::dispatch::{AttentionMaskKind, KernelPath, attention_forward_candle};
use aarambh_studio_kernel::{
...
fn attention_mask_kind_recognizes_project_causal_shape() {
    ...
    assert_eq!(
        aarambh_studio_kernel::dispatch::attention_mask_kind(&q, &k, Some(&mask)),
...
fn cuda_phase14_wrappers_report_kernel_availability() {
    assert_eq!(
        aarambh_studio_kernel::flash_attn::cuda_kernels_compiled(),
        cfg!(aarambh_cuda_kernels)
    );
    assert_eq!(
        aarambh_studio_kernel::fused_norm::cuda_kernels_compiled(),
        cfg!(aarambh_cuda_kernels)
    );
    assert_eq!(
        aarambh_studio_kernel::fused_rope::cuda_kernels_compiled(),
        cfg!(aarambh_cuda_kernels)
    );
    assert_eq!(
        aarambh_studio_kernel::fused_ffn::cuda_kernels_compiled(),
        cfg!(aarambh_cuda_kernels)
    );
}

...
pub fn fused_rms_norm(x: &Tensor, weight: &Tensor, eps: f32) -> Result<Tensor> {
    ...
    #[cfg(not(all(feature = "cuda", aarambh_cuda_kernels)))]
    {
        ...
        Err(candle_core::Error::msg(
            "Fused CUDA RMSNorm is unavailable because aarambh-studio-kernel was built without CUDA PTX kernels",
        ))
    }
}
...
mod cuda {
    ...
    const PTX: &str = include_str!(env!("AARAMBH_STUDIO_CUDA_RMS_NORM_PTX"));

...
/// Run the CUDA fused RoPE application kernel.
pub fn fused_rope_apply(
    ...
    #[cfg(not(all(feature = "cuda", aarambh_cuda_kernels)))]
    {
        ...
        Err(candle_core::Error::msg(
            "Fused CUDA RoPE is unavailable because aarambh-studio-kernel was built without CUDA PTX kernels",
        ))
    }
}
...
mod cuda {
    ...
    const PTX: &str = include_str!(env!("AARAMBH_STUDIO_CUDA_ROPE_PTX"));

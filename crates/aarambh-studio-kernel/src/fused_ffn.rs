...
pub fn fused_swiglu(gate: &Tensor, up: &Tensor) -> Result<Tensor> {
    ...
    #[cfg(not(all(feature = "cuda", aarambh_cuda_kernels)))]
    {
        ...
        Err(candle_core::Error::msg(
            "Fused CUDA SwiGLU is unavailable because aarambh-studio-kernel was built without CUDA PTX kernels",
        ))
    }
}
...
mod cuda {
    ...
    const PTX: &str = include_str!(env!("AARAMBH_STUDIO_CUDA_SWIGLU_PTX"));
